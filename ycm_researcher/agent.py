        ...
        if self.log_handler:
            try:
                if event_type == "tool":
                    ...
                    await self.log_handler.on_tool_start(tool_name, **tool_kwargs)
                elif event_type == "action":
                    action = kwargs.get('action', '')
                    action_kwargs = {k: v for k, v in kwargs.items() if k != 'action'}
                    await self.log_handler.on_agent_action(action, **action_kwargs)
